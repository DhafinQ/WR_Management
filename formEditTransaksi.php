...
$barangs = $controllers->getBarangData($conn);
$transaksi = $controllers->getTransaksi($conn,$_GET['id']);
?>
<html lang="en">
...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl">
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            <a href="indexPemasok.php" class="flex items-centertext-white active-nav-link py-4 pl-6 nav-item">
            ...
            <main class="w-full flex-grow p-6">
                ...
                <nav aria-label="breadcrumb" class="w-max">
                    <ol class="flex w-full flex-wrap items-center rounded-md bg-slate-50 px-4 py-2">
                        ...
                        <li class="flex cursor-pointer items-center text-sm text-slate-500 transition-colors duration-300 hover:text-slate-800">
                            <a href="indexTransaksi.php">Transaksi</a>
                            <span class="pointer-events-none mx-2 text-slate-800">
                                /
                            </span>
                        </li>
                        <li class="flex cursor-pointer items-center text-sm text-slate-500 transition-colors duration-300 hover:text-slate-800">
                            <a href="javascript:void()">Edit Transaksi</a>
                        </li>
                    </ol>
                </nav>
                ...
                <div class="w-full mt-12">
                    <p class="text-xl pb-6 flex items-center">
                        <i class="fas fa-list mr-3"></i> Form Edit Transaksi
                    </p>
                    <div class="leading-loose">
                        <form class="p-10 bg-white rounded shadow-xl" id="transaksiForm">
                            <input type="hidden" name="model" value="transaksi">
                            <input type="hidden" name="id" value="<?= $transaksi['id']?>">
                            <div class="">
                            <label class="block text-sm text-gray-600" for="lokasi">Transaksi</label>
                                <select
                                    name="id_barang"
                                    class="w-full bg-gray-200 placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer">
                                    <option value="">-- Pilih --</option>
                                    <?php
                                    if (!empty($barangs)):
                                        foreach ($barangs as $barang):
                                    ?>
                                            <option value="<?= $barang['id']; ?>" <?= ($barang['id'] == $transaksi['id_barang'] ? 'selected' : '') ?>><?= $barang['nama_barang']; ?></option>
                                    <?php endforeach;
                                    endif; ?>
                                </select>    
                            </div>
                            <div class="">
                                ...
                                <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" value="<?=$transaksi['jumlah']?>" id="jumlah" name="jumlah" type="number" required="" placeholder="Masukan alamat pemasok..." aria-label="Name">
                            </div>
                            <div class="">
                            <label class="block text-sm text-gray-600" for="jumlah">Tipe Transaksi</label>
                                <select
                                    name="tipe_transaksi"
                                    class="w-full bg-gray-200 placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer" required>
                                        <option value="">-- Pilih --</option>
                                        <option value="penjualan" <?= ($transaksi['tipe_transaksi'] == 'penjualan') ? 'selected': ''?>>Penjualan</option>
                                        <option value="pembelian" <?= ($transaksi['tipe_transaksi'] == 'pembelian') ? 'selected': ''?>>Pembelian</option>
                                </select>
                            </div>
                            <div class="">
                                <label class="block text-sm text-gray-600" for="tgl_transaksi">Tanggal Transaksi</label>
                                <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" value="<?=$transaksi['tgl_transaksi']?>" id="tgl_transaksi" name="tgl_transaksi" type="date" required="" aria-label="kapasitas">
                            </div>
                ...
                success: function(response) {
                    if (response == 'success') {
                        Swal.fire({
                            ...
                            window.location = 'indexTransaksi.php'; // Redirect to index after alert
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
